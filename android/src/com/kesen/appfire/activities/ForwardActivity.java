...
import com.kesen.appfire.views.DevlomiSnackbar;
...
public class ForwardActivity extends AppCompatActivity {
    ...
    private void handleIncomingAudio() {
        ...
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            for (User user : adapter.getSelectedForwardedUsers()) {
                Message message = MessageCreator.createAudioMessage(user, filePath, length);
    ...
    private void handleVideoShare() {
        ...
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            ...
            for (User user : adapter.getSelectedForwardedUsers()) {
                Message message = MessageCreator.createVideoMessage(this, user, filePath);
                ServiceHelper.startNetworkRequest(this, message.getMessageId());
                showSendingToast();
                finish();
            }
            ...
            if (selectedUsers.size() > 1) {
                for (User user : selectedUsers) {
                    ...
                    for (Uri uri : imageUris) {
                        String filePath = RealPathUtil.getRealPath(this, uri);
                        if (filePath != null) {
                            Message message = MessageCreator.createImageMessage(user, filePath, false);
                            ServiceHelper.startNetworkRequest(this, message.getMessageId());
                        } else {
                            showFileNotFoundToast();
                        }
    ...
    private void handleTextShare(List<User> selectedUsers) {
        ...
        if (selectedUsers.size() > 1) {
            for (User selectedUser : selectedUsers) {
                Message message = MessageCreator.createTextMessage(selectedUser, sharedText);
                ServiceHelper.startNetworkRequest(this, message.getMessageId());
            }
            showSendingToast();
            finish();

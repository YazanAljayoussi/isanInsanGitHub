...
public class NewGroupActivity extends ForwardActivity implements ForwardAdapter.OnUserClick, NewGroupSelectedUsersAdapter.OnUserClick, ForwardActivity.SearchCallback {
    ...
    public void onQuery(String newText) {
        if (!newText.trim().isEmpty()) {
            ...
            allUsersAdapter = new NewGroupAdapter(users, selectedForwardedUsers, currentGroupUsers, true, this, this);
            rvGroup.setAdapter(allUsersAdapter);
        } else {
            allUsersAdapter = new NewGroupAdapter(this.users, selectedForwardedUsers, currentGroupUsers, true, this, this);
            rvGroup.setAdapter(allUsersAdapter);
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (groupId != null) {
            ...
            currentGroupUsers = user.getGroup().getUsers();
    ...
    public void onChange(User user, boolean added) {
        ...
        if (added) {
            if (count + EXTRA_COUNT > MAX_GROUP_USERS_COUNT) {
    ...
    private void updateSelectedUsers(int count) {
        ...
        if (count == 0) {
            ...
            tvAddParticipantsTvToolbar.setText(count + " of " + MAX_GROUP_USERS_COUNT);
        }
    }

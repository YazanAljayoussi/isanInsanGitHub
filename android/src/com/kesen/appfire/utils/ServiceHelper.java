...
import com.kesen.appfire.job.NetworkJobService;
import com.kesen.appfire.job.SaveTokenJob;
import com.kesen.appfire.job.SyncContactsJob;
...
import com.kesen.appfire.services.SyncContactsService;
...
public class ServiceHelper {
    ...
    public static void startNetworkRequest(Context context, String messageId) {
        String action = IntentUtils.INTENT_ACTION_NETWORK_REQUEST;

        if (Util.isOreoOrAbove()) {
            PersistableBundle bundle = new PBundleUtil.Builder(messageId).action(action).messageId(messageId).build().getBundle();
    ...
    public static void startUpdateMessageStatRequest(Context context, String messageId, String myUid, int statToBeUpdated) {
        if (Util.isOreoOrAbove()) {
    ...
    public static void startUpdateVoiceMessageStatRequest(Context context, String messageId, String myUid) {
        if (Util.isOreoOrAbove()) {
    ...
    public static void fetchUserGroups(Context context) {
        if (Util.isOreoOrAbove()) {
            PersistableBundle bundle = new PBundleUtil.Builder(UUID.randomUUID().toString()).
                    action(IntentUtils.INTENT_ACTION_FETCH_USER_GROUPS).
                    build()
                    .getBundle();
            ...
            Intent intent = new Intent(context, NetworkService.class);
            intent.setAction(IntentUtils.INTENT_ACTION_FETCH_USER_GROUPS);
            context.startService(intent);
        }
    }

...
import com.kesen.appfire.views.SetGroupTitleDialog;
...
public class UserDetailsActivity extends AppCompatActivity implements GroupParticipantsAdapter.OnParticipantClick, AppBarLayout.OnOffsetChangedListener {
    ...
    private RecyclerView rvGroupParticipants;
    ...
    protected void onStop() {
        super.onStop();
        //free up resouces and avoid memory leaks
        onGetUserPhoto = null;
    }
    ...
    private void setAdapter() {
        ...
        if (isGroup) {
            groupParticipantsAdapter = new GroupParticipantsAdapter(this, user.getGroup().getAdminsUids(), userList, this);
            rvGroupParticipants.setLayoutManager(new LinearLayoutManager(this));
            rvGroupParticipants.setAdapter(groupParticipantsAdapter);
            appBar.addOnOffsetChangedListener(this);
        }
    }


    private void showGroupLayouts() {
    ...
    private void loadUserImageLocally() {
        if (user.getUserLocalPhoto() != null && FileUtils.isFileExists(user.getUserLocalPhoto())) {
            Glide.with(this).load(user.getUserLocalPhoto()).into(userImageToolbar);
        } else if (user.getThumbImg() != null) {
            byte[] bytes = BitmapUtils.encodeImageAsBytes(user.getThumbImg());
            Glide.with(this).load(bytes).asBitmap().into(userImageToolbar);
        }
    }
    ...
    private void init() {
        ...
        rvGroupParticipants = findViewById(R.id.rv_group_participants);
        ...
        addParticipantAdmin = findViewById(R.id.add_participant_admin);
    ...
    private void editGroupClicked() {
        ...
        setGroupTitleDialog.setmListener(new SetGroupTitleDialog.OnFragmentInteractionListener() {
            @Override
            public void onPositiveClick(final String groupTitle) {

                //if it's the same name do nothing
                if (groupTitle.equals(user.getUserName()))
                    return;
                ...
                progressDialog.setCancelable(false);
                progressDialog.show();
                 GroupManager.changeGroupName(groupTitle, user.getUid(), new GroupManager.OnComplete() {
                    @Override
                    public void onComplete(boolean isSuccessful) {
                        progressDialog.dismiss();
                        if (isSuccessful) {
    ...
    public void onLongClick(final User user, View view) {
        if (isAdmin) {
            ...
            popup.getMenuInflater()
                    .inflate(R.menu.popup_menu_admin_options, popup.getMenu());


            String userName = user.getUserName();
            final boolean isSelectedUserIsAdmin = FireManager.isAdmin(user.getUid(), UserDetailsActivity.this.user.getGroup().getAdminsUids());
                ...
                public boolean onMenuItemClick(MenuItem item) {
                    switch (item.getItemId()) {
                        ...
                        case R.id.make_group_admin:
                            makeGroupAdmin(user, !isSelectedUserIsAdmin);
                            break;
